...
const sendProject = () => win.webContents.send('project:changed', project?.snapshot(activePage, pageSize) || null)
...
ipcMain.handle('project:create', async (_, name) => { const choice = await dialog.showSaveDialog(win, { title: 'Create NER project', defaultPath: `${name || 'untitled'}.nerdb`, filters: [{ name: 'NER project', extensions: ['nerdb'] }] }); if (choice.canceled || !choice.filePath) return null; project?.close(); project = new ProjectDatabase(choice.filePath); activePage = 0; project.initialize(name || 'Untitled project'); const snapshot = project.snapshot(activePage, pageSize); rememberProject(snapshot.path, snapshot.name); sendProject(); return snapshot })
ipcMain.handle('project:open', async () => { const choice = await dialog.showOpenDialog(win, { properties: ['openFile'], filters: [{ name: 'NER project', extensions: ['nerdb'] }] }); if (choice.canceled || !choice.filePaths[0]) return null; project?.close(); project = new ProjectDatabase(choice.filePaths[0]); activePage = 0; const snapshot = project.snapshot(activePage, pageSize); rememberProject(snapshot.path, snapshot.name); sendProject(); return snapshot })
ipcMain.handle('project:recent', () => recentProjects())
ipcMain.handle('project:open-recent', (_, filePath) => { if (!fs.existsSync(filePath)) { recentProjects(); throw new Error('This project file is no longer available.'); } project?.close(); project = new ProjectDatabase(filePath); activePage = 0; const snapshot = project.snapshot(activePage, pageSize); rememberProject(snapshot.path, snapshot.name); sendProject(); return snapshot })
ipcMain.handle('project:page', (_, page) => { requireProject(); activePage = Math.max(0, page); return project.snapshot(activePage, pageSize) })
...
ipcMain.handle('settings:set-provider', (_, provider) => { if (!['openai', 'gemini'].includes(provider)) throw new Error('Choose a valid provider.'); writeSettings({ ...readSettings(), provider }); return provider })
